...
namespace QuantBox
{
    ...
    {
        private class TradingProcessor
        {
            ...
            {
                _provider._logger.Info(field.DebugInfo);
            ...
            {
                _provider._logger.Info(string.Join(",", order.Id, order.Side, order.Instrument.Symbol, order.Qty, order.Price));
                var localId = _provider._trader.SendOrder(CreateOrderField(order));
            ...
            public TradingProcessor(XProvider provider)
            {
                _provider = provider;
                InitHandler();
            }
